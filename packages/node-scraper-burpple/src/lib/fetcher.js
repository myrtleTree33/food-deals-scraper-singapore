...
export const scrapePage = async (
  ...
  return $('#masonry-container > div')
    .map((i, elem) => {
      ...
      const hasBeyond = $(
        'div.searchVenue-reasonTags.card-item > a:nth-child(2) > div',
        elem
      )
        .text()
        .includes('BEYOND');
      ...
      let id = $(
        'div.searchVenue-header.card-item.card-item--header > a',
        elem
      ).attr('href');
      ...
      const reviews = $(
        ...
        .map((i, elem2) => {
          const content = $(
            'div.searchVenue-reviews-container-items-item-body-desc',
            elem2
          )
            .text()
            .replace(/\n/g, '');
...
export const scrapeEntry = async (opts = { url: undefined }) => {
  ...
  const location = [
    canvas.attr('data-venue-lng'),
    canvas.attr('data-venue-lat')
  ];
  ...
  let openingHours = $(
    '#venueInfo-details-header-item-body-hidden-openingHours > p'
  )
    .map((i, elem) => {
      const text = $(elem)
        .text()
  ...
  openingHours = [...currOpeningHours, ...openingHours].filter(
    x => !(x === '-' || x === '')
  );
